				...
				<button
					...
					onClick={async () => {
						...
						const formatedIngredients = (selectedRecipe.ingredients || []).map((ingredient: any) => {
							const multiplier = getMultiplier(unitsList, ingredient.unit, ingredient.unit === "count" ? "each" : ingredient.baseUnit);
							const ingredientCost = ingredient.unit === "each" ? ingredient.price*(Number(ingredient.amount) || 0) : ingredient.unit === "count" ? ingredient.price/(Number(ingredient.amount) || 0) : (Number(ingredient.costPerUnit) || 0) * (Number(ingredient.amount) || 0) * (multiplier);
